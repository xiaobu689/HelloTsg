...
class Ctrip:
    ...
    def getMainCookie(Cookie, readerId):
        ...
        query = f"&fromminiapp=weixin&allianceid={maindic['AID']}&sid={maindic['SID']}&sourceid={maindic['SourceID']}&_cwxobj=" + str(
            _cwxobj)
        maindic['query'] = query
        return maindic
    ...
    def getheaders(Cookie, mainCK):
        str = 'm.ctrip.com/activitysetupapp/mkt/index/membersignin2021?isHideNavBar=YES&pushcode=miniprogram"' + mainCK[
            'query']
    ...
    def Sign(headers, mainCK):
        ...
        data = {"head": {"cid": str(mainCK['GUID']), "ctok": "", "cver": "1.0", "lang": "01", "sid": "8888",
        ...
        try:
            ...
            if (res['sign']) == False:
                ...
                rmsToken = "fp=sqkm95-1poj0zf-1uks0cc" + "&vid=" + str(mainCK["vid"]) + "&r=" + str(
                    mainCK["RGUID"]) + "&ip=" + str(mainCK["ip"]) + "&ua=" + urllib.parse.quote(str(headers[
                                                                                                        "User-Agent"])) + "&pageId=10650004935&rg=fin&screen=414x896&tz=+8&blang=zh-CN&oslang=zh-CN&v=m17&bl=false&clientid="
                body = {
                ...
                if (res["code"] == 200):
                    for item in items:
                        ...
                        try:
                            ...
                            listurl = "https://m.ctrip.com/restapi/soa2/16189/json/moduleListSearch?_fxpcqlniredt=" + \
                                      mainCK['GUID']
                            listbody = {
                                "lat": 0,
                                "lon": 0,
                            ...
                            ruleurl = "https://m.ctrip.com/restapi/soa2/14045/json/ruleSortCommentList?" + mainCK[
                                'GUID']
                            rulebody = {
                                "articleId": id,
                            ...
                            rtRdurl = "https://m.ctrip.com/restapi/soa2/14045/json/relatedRecommend?_fxpcqlniredt=" + \
                                      mainCK['GUID']
                            rtRdbody = {
                                "articleId": id,
                                "head": head
                            }
                            shimurl = "https://m.ctrip.com/restapi/soa2/15416/json/getShareImage?_fxpcqlniredt=" + \
                                      mainCK['GUID']
                            shimbody = {
                                "businessId": id,
                            ...
                            rMkturl = "https://m.ctrip.com/restapi/soa2/20725/json/reportMktProductClick?" + mainCK[
                                'GUID']
                            rMktbody = {
                            ...
                            if (Mkt["result"]["result"] == True):
                                ...
                                taskbody = {
                                    "_taskDetailId": "pages/you/lvpai/detail/detail?_mktTaskActivityId=317sqhb&wxpopup=201&inpopup=true&articleId={0}".format(
                                        id),
                                ...
                                if (check["code"] == 200):
                                    ...
                                    time.sleep(2)
                                    riskurl = "https://m.ctrip.com/restapi/soa2/20725/json/getRiskInfo?" + mainCK[
                                        'GUID']
                                    riskbody = {"head": head}
                                    risk = requests.post(url=riskurl, headers=headers, data=json.dumps(riskbody)).json()
                                    if (risk["riskResult"]["riskMessage"] == "PASS"):
                                        time.sleep(13)
                                        taskurl = "https://m.ctrip.com/restapi/soa2/22598/taskBrowseDone? " + mainCK[
                                            'GUID']
                                        done = requests.post(url=taskurl, headers=headers,
                                                             data=json.dumps(taskbody)).json()
                                        if (done["code"] == 200):
                                ...
                                print("请检查Mkt请求body！")
                            n = n + 1
                            queryurl = "https://m.ctrip.com/restapi/soa2/16225/json/getContentFissionCashIndex?_fxpcqlniredt=" + \
                                       mainCK['GUID']
                            querybody = {
                                "env": 1,
